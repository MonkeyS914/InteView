...
// manner of such use.
//
// $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BStreamFileToolkit.cpp,v 1.291 2009-01-23 07:00:16 reuben Exp $
//
...
#include "stdlib.h"
#define  LOG_TAG    "BStreamFileToolkit"   //定义logcat中tag标签
...
{
	//if (!strncmp (block, ";; IVP V", 8))
	if (!(strncmp (block, ";; IVP V", 8)&&strncmp (block, ";; HSF V", 8)))
	{
...
TK_Status BStreamFileToolkit::AppendLastKey (ID_Key key) alter {
	...
	if (m_last_keys_used >= m_last_keys_allocated) {
		...
		if ( m_last_keys_allocated < 4194304 )	
			m_last_keys_allocated *= 2;
		else
			m_last_keys_allocated += 1048576;		
...
bool BStreamFileToolkit::add_exref (Internal_ExRef_List * exref) alter {
	...
	while (caller != null) {
		if (!wcscmp (exref->Reference(), caller->Reference())) {	
			delete exref;
			return false;
		}
...
TK_Status BStreamFileToolkit::Read_Stream_File () {
    ...
    if ((status = OpenFile (filename)) == TK_Normal){
				//m_external_references->SetActual (wfilename);
    }else {
        if (first_file)
            return status;
        wchar_t const *
        bare = wcsrchr2 (wfilename, '/', '\\');
        if (bare != null)
            status = OpenFile (++bare);
        else
            bare = wfilename;
        if (status == TK_Normal)
            ...
            if (caller != null) do if (caller->Actual() != null) {
                wchar_t *trial = new wchar_t [wcslen(caller->Actual()) + barelen];
