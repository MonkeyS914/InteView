 ...
 * manner of such use.
 *
 * $Header: /files/homes/master/cvs/hoops_master/shared/stream_common/BEdgeBreakerCompress.cpp,v 1.128 2008-11-20 21:29:42 jason Exp $
 * 
    ...
    int alter *newpcount_out, loop_table alter *ltable, int alter *remap, vhash_t alter **tristrip_hash_out )  
{
    ...
    loops_per_piece = (int *) malloc ( (newpcount+1) * sizeof( int ) );  
                ...
                eb_compress_configs const *configs_in )
{
    ...
    while( j < 3 ) {
        status = compute_opcodes( 
            &ea, configs.hints, pcount, pcount_max,		
            &newpcount,                                 
